    ...
    if (-e "$outpath/total_coverage_depth_stat"){
        ...
        while (<TOTAL>){
            ...
            if($t_sample eq $sample_name2){
				...
				$depth=(split /\t/,$line)[4];
				my $q30=(split /\t/,$line)[3];
				if ($depth > 80 and $q30>=85 ){ #add q30 check 20191231
	  ...
	  #      }else{
	   ...
	   #     }
		...
		if ($product=~/BGI59/) {
		    ...
		    $tag=1 if $depth >100 or $depth2>=100;  #20190905
		}elsif($product=~/PP/){
             $tag=1 if $depth >=120 or $depth2 >=120; #20191231
		}else{
                    $tag=0;
        }
	if ($tag == 1) {
		if ($outpath=~/BGISEQ/ or $outpath=~/Zebra/ or $outpath=~/MGISEQ-2000/){
        	#print OUT1 "perl /ifs9/B2C_SGD/PROJECT/MGISEQ-2000_Project/exome_diagnose/exome_cnv/WebServer/src/new_exon_picture_picture_500.pl $gene $outpath/$sample_name2/coverage/depth.tsv.gz\n";
		print OUT1 "perl /ifs9/B2C_SGD/PROJECT/MGISEQ-2000_Project/exome_diagnose/exome_cnv/WebServer/src/new_exon_picture_picture_2000.pl $gene $outpath/$sample_name2/coverage/depth.tsv.gz\n";  #20190905
	}else{
